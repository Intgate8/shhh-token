...
contract ShhhStakingLPContract is Context, Owner {
    ...
    */
    function stake(uint256 _amount) public payable returns(bool) {
        require(LPContract.balanceOf(_msgSender()) >= _amount,'Insufficient number of tokens');

        require(LPContract.transferFrom(_msgSender(),address(this), _amount));
    ...
    */
    function unstake(uint256 _amount) public returns(bool) {
        ...
        uint256 erc20balance = LPContract.balanceOf(address(this));
        require(_amount <= erc20balance, "Balance is too low");

        setNewClaim(_msgSender());
        //token.approve(_msgSender(), _amount);
        require(LPContract.transfer(_msgSender(), _amount));
